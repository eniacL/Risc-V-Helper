...
class HdboTrans {
    async provideHover(document, position) {
        ...
        if (regexBin.test(hoveredWord.toString())) {
            ...
            hoveredWord = hoveredWord.replace("B", "");
            let input = Number(parseInt(hoveredWord, 2).toString());
            let input_b = input.toString(2).replace(/(^\s+|\s+$)/, '');
            ...
            hoveredWord = hoveredWord.replace("H", "");
            // console.log(hoveredWord);
            let input_b = parseInt(hoveredWord, 16).toString(2).replace(/(^\s+|\s+$)/, '');
            ...
            hoveredWord = hoveredWord.replace("D", "");
            let input = Number(hoveredWord.toString());
            let input_b = input.toString(2).replace(/(^\s+|\s+$)/, '');
...
exports.HdboTrans = HdboTrans;
//# sourceMappingURL=HdboTrans.js.map